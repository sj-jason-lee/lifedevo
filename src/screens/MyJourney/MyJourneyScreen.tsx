...
export function MyJourneyScreen() {
  const { user, completions, journalEntries, prayers, devotionals, getUserPrayers, togglePrayerAnswered } =
    useAppContext();
  ...
  const nextMilestone = MILESTONES.find((m) => m > (user?.streakCount || 0)) || 365;
  const prevMilestone = [...MILESTONES].reverse().find((m) => m <= (user?.streakCount || 0)) || 0;
    ...
    <ScrollView contentContainerStyle={styles.tabContent} showsVerticalScrollIndicator={false}>
      ...
      <Card style={styles.streakCard}>
        ...
        <View style={styles.streakProgressBar}>
          <View
            style={[
              ...
              {
                width: `${Math.min(
                  (((user?.streakCount || 0) - prevMilestone) / (nextMilestone - prevMilestone)) * 100,
                  100
                )}%`,
              },
            ]}
        ...
        <Text style={styles.milestoneText}>
          {nextMilestone - (user?.streakCount || 0)} days until {nextMilestone}-day milestone
        </Text>
      </Card>
      ...
      <View style={styles.statsGrid}>
        ...
        <Card style={styles.statCard}>
          <Ionicons name="hand-left-outline" size={24} color={colors.prayerBlue} />
      ...
      <View style={styles.milestonesSection}>
        ...
        <View style={styles.milestoneRow}>
          {MILESTONES.map((milestone) => {
            ...
            return (
              <View key={milestone} style={[styles.milestoneItem, reached && styles.milestoneReached]}>
                <Ionicons
                  name={reached ? 'checkmark-circle' : 'ellipse-outline'}
                  size={24}
                  color={reached ? colors.success : colors.borderLight}
                />
                <Text style={[styles.milestoneNumber, reached && styles.milestoneNumberReached]}>
                  {milestone}
                </Text>
  ...
  const renderJournal = () => (
    <FlatList
      data={userJournals.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())}
      renderItem={({ item }: { item: JournalEntry }) => (
        <Card style={styles.entryCard}>
          <View style={styles.entryHeader}>
            <View style={styles.scriptureRefBadge}>
              <Ionicons name="book-outline" size={12} color={colors.primary} />
              <Text style={styles.scriptureRefText}>{getDevotionalRef(item.devotionalId)}</Text>
            </View>
      ...
      ListEmptyComponent={
        <View style={styles.emptyContainer}>
          <Ionicons name="pencil-outline" size={48} color={colors.borderLight} />
          <Text style={styles.emptyTitle}>No Journal Entries Yet</Text>
          <Text style={styles.emptyMessage}>
            Start journaling during your daily devotionals to build a record of your spiritual journey.
          </Text>
        </View>
      }
    />
  ...
  const renderPrayers = () => (
    <FlatList
      data={userPrayers.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())}
        ...
        <Card style={[styles.entryCard, styles.prayerCard]}>
          <View style={styles.entryHeader}>
            <View style={[styles.scriptureRefBadge, styles.prayerRefBadge]}>
              <Ionicons name="book-outline" size={12} color={colors.prayerBlue} />
              <Text style={[styles.scriptureRefText, { color: colors.prayerBlue }]}>
      ...
      ListEmptyComponent={
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyMessage}>
            Write prayers during your devotional time to keep a record of how God is working in your life.
          </Text>
        </View>
      }
    />
        ...
        {(['overview', 'journal', 'prayers'] as TabType[]).map((tab) => (
          ...
          >
            <Text style={[styles.tabText, activeTab === tab && styles.activeTabText]}>
              {tab === 'overview' ? 'Overview' : tab === 'journal' ? 'Journal' : 'Prayers'}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
